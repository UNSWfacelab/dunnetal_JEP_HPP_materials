...
numberOfTargets = {4 4 6 6 8 8}; %enter the number of targets per trial you want for each block 
numberOfTargetIdentity = {4 4 6 6 8 8}; %enter the number of different target images for each trial. Must be a factor of the number of targets
targetDisplayTime = {4 4 4 4 4 4}; %how long the target image is shown for in seconds
randomiseNumberOfTargets = {0 0 0 0 0 0}; %0 = no, 1 = yes
feedback = {0 0 0 0 0 0}; %feedback, 0 = no, 1 = yes
targetPrevalence = {1/2 1/2 1/2 1/2 1/2 1/2};
targetDelay = {1 1 1 1 1 1};
...
maskLocation = 'twirled/u05tp.jpg';
...
for block = blockList
    ...
    for trial = 1:numberOfTrials{block}
        ...
        elseif targetType(trial)  == 1
            probTexture(end+1) = Screen('MakeTexture', window, targetImages{targetTested(trial)});
        end
...
%         DrawFormattedText(window, trialText, XposHeading, YposHeading, white, wrapat, [], [], vSpacing);
        Screen('DrawLine', window, black, X0-widthOfFix/2, Y0, X0+widthOfFix/2, Y0,lineWidth);
        Screen('DrawLine', window, black, X0, Y0-heightOfFix/2, X0, Y0+heightOfFix/2,lineWidth);
        Screen('DrawTextures', window, probTexture, [], targetArrayPosition{trialTargets}(:,targetTested(trial)));
        Screen('Flip',window);
        tstart = tic;
        ...
        %% ----------------- Data Saving ---------------------------
                                ...
                                pos=pos+1;  DATAmatrix(experimentTrial,pos) = targetTested(trial);                          heading{pos} = 'Target Position Changed';
                                ...
                                pos=pos+1;  DATAmatrix(experimentTrial,pos) = foilImageNumber;                              heading{pos} = 'Foil Image Number'; 
